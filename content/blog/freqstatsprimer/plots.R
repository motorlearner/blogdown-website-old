...
plot.lln <- replicate(20, cumsum(rbinom(10000, 1, 0.5))) |> 
  as.data.frame() |> 
  mutate(n = row_number()) |> 
  filter(n <= 1000 | n >= 9000) |> 
  mutate(c = ifelse(n <= 1000, 1, 2)) |> 
  pivot_longer(cols = starts_with("V"),
               values_to = "p", 
               names_to  = "id") |> 
  ggplot(aes(x=n, y=p/n, group=id)) +
  geom_line(size=0.1) +
  facet_grid(cols=vars(c), scales="free_x") +
  scale_x_continuous(name = "# Flips", expand=expansion(0), breaks=c(1,seq(0,10000,500))) +
  scale_y_continuous(name = "Proportion Heads", expand=expansion(0)) +
  theme_blog() +
  theme(panel.grid.major=element_blank(),
        panel.border=element_blank(),
        strip.text=element_blank(),
        plot.margin=unit(c(.8, 1.2, .8, .2), "lines"),
        panel.spacing=unit(3, "lines")); plot.lln
...
plot.overview <- ggplot() + theme_void() + theme(plot.margin = unit(c(.8,.2,.8,.2), "lines")) +
  # axis limits
...
plot.cltsamp <- list( # create dataframes
...
plot.cltdist <- list( # create dataframes
...
## sampling distribution placement ------------------------------------------------------

plot.sdistplace <- getnd(0,1/sqrt(15)) |> 
  ggplot(aes(y=y)) +
  # data
  geom_area(aes(x=x-6.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-6.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-5.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-5.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-4.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-4.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-3.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-3.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-2.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-2.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-1.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-1.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x-0.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x    ), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x+0.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x+1.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x+1.5), size=0.1, color=c[2], fill=NA, alpha=0.1) +
  geom_area(aes(x=x+2.0), size=0.1, color=c[2], fill=NA, alpha=0.1) +
...
plot.sdistplace1 <- plot.sdistplace +
  geom_area(aes(x=x), color=c[2], fill=c[2]) +
  geom_vline(xintercept=0, linetype="dashed") +
  geom_vline(xintercept=mean(sample), linetype="solid") 

plot.sdistplace2 <- plot.sdistplace +
  geom_area(aes(x=x-0.5), color=c[2], fill=c[2]) +
  geom_vline(xintercept=-0.5, linetype="dashed") +
  geom_vline(xintercept=mean(sample), linetype="solid") 
  

ggsave(
  plot     = plot.sdistplace1,
  filename = "sdistplace1.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 1.8,
  width    = 7
...
ggsave(
  plot     = plot.sdistplace2,
  filename = "sdistplace2.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 1.8,
  width    = 7
...
plot.pvalcurve <- p1 + p1 + p1 + p4 + p5 + p6 + p7 + p8 + p9 + plot_layout(design=layout)

ggsave(
  plot     = plot.pvalcurve,
  filename = "pvaluecurve.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 8,
  width    = 10
...
plot.ci <- p1 + p1 + p1 + p4 + p5 + p6 + p7 + p8 + p9 + plot_layout(design=layout) 
...
plot.cibyn <- p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9 + plot_layout(design=layout)
...
plot.rejrate <- p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9 + plot_layout(design=layout)

ggsave(
  plot     = plot.rejrate,
  filename = "rejrate.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 8.5,
  width    = 10
...
plot.rejrateinference <- p1 + p2 + p3 + p4 + p5 + p6 + plot_layout(design=layout)

ggsave(
  plot     = plot.rejrateinference,
  filename = "rejrate_inference.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 6.7,
  width    = 10
...
plot.rejratebyn <- p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9 + plot_layout(design=layout)

ggsave(
  plot     = plot.rejratebyn,
  filename = "rejrate_n.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 8.2,
  width    = 10
...
plot.rejratebyalpha <- p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9 + plot_layout(design=layout)

ggsave(
  plot     = plot.rejratebyalpha,
  filename = "rejrate_alpha.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 7,
  width    = 10
...
ggsave(
  plot     = plot.rejrateheat,
  filename = "rejrateheat.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 4,
  width    = 10
...
plot.rejrateheatbyalpha <- expand.grid(
  n = seq(1, 100, 1),
...
ggsave(
  plot     = plot.rejrateheatbyalpha,
  filename = "rejrateheat_alpha.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 3.5,
  width    = 10
...
plot.rejrateheatbysigma <- expand.grid(
  n = seq(1, 100, 1),
  alpha = 0.05,
  sigma = c(0.5, 1, 1.5),
  mu = seq(-3, 3, 0.01)
...
ggsave(
  plot     = plot.rejrateheatbysigma,
  filename = "rejrateheat_sigma.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 4,
  width    = 10
...
plot.rejrateheatbysigmastand <- expand.grid(
  n = seq(1, 100, 1),
  alpha = 0.05,
  sigma = c(0.5, 1, 1.5),
  z = seq(-2, 2, 0.01)
...
ggsave(
  plot     = plot.rejrateheatbysigmastand,
  filename = "rejrateheat_sigma_std.svg",
  path     = plotdir,
  scale    = 0.6,
  height   = 4,
  width    = 10
