...
gifdir  <- here(plotdir, "gif_rejrate_h1")
fps     <- 20
sec     <- 4
nframes <- fps*sec+1 # +1 -> odd -> includes a frame at mu0

xvec1a <- seq(0, +3, .01) # for computing rejection rate
xvec1b <- seq(-3, 0, .01)
xvec2  <- seq(-3,+3, .01)
...
xrun_init0_1a <- seq(0, +3, length.out=nframes)
xrun_init1_1a <- gifsec(xrun_init0_1a, minfac=.05, mode="sf", halfwidth=2*sem, addvalues=rep(0,20))
xrun1a <- rep(xrun_init1_1a, 2)
xrun1b <- -xrun1a
xrun2  <- c(xrun_init1_1a, -xrun_init1_1a)
...
for (i in seq_along(xrun1a)) {
  ...
  savepng(plot=p, filename=paste0("rejrateH1", sprintf("%03d", i)), w=12, h=6.5, directory = gifdir)
...
gifski(pngs, gif_file=paste0(plotdir, "/rejrateH1.gif"), delay=1/fps, width=1200, height=650)